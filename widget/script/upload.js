...
!function(window){
	var u = {
		...
		baseUrl: 'http://testgole.milianjie.com/index.php/api/',//测试地址
//		baseUrl: 'http://testweidianpu.milianjie.com/index.php/api/',
		//获取七牛上传token的请求地址
		qiNiuPrefixUrl: 'http://7xj8rh.com1.z0.glb.clouddn.com',//七牛上的文件地址前缀
		qiNiuUploadUrl: IS_FORMAL_ENV ? 'http://up-na0.qiniu.com' : 'http://up-na0.qiniu.com',//七牛上传地址
...
//								});
						...
						if(err && err.code==2){
							...
							if('none' === api.connectionType){
								api.toast({
									msg: 'The network cannot be connected. Please check whether the network device is normal.',
									location: 'bottom',
									duration: 3000
								});
								return;
...
//								});		
							...
							if(ret){
								api.toast({
									msg: ret && ret.body && ret.body.error||ret.body,
									location: 'bottom',
									duration: 3000
								});
							}else{
								api.toast({
									msg: err && err.body && err.body.error||'unknown error',
									location: 'bottom',
									duration: 3000
								});
							}
							...
							/*上传中状态:uploading*/
							if(args.callbackEvent){
								self.uploading(args.callbackEvent, ret && ret.progress || '0');
//								api.sendEvent({
//									name: args.callbackEvent,
//									extra: {
...
//								});								
		...
		uploading: function(callbackEvent, progress) {
			...
			$api.text(statusDom, progress >0 ? progress.toFixed(2) + '%':0 + '%');
		...
		error: function(callbackEvent, url) {
			...
			$api.text(statusDom, 'Upload failed, click upload again ');
			if(closeDom) {
				$api.removeCls(closeDom, 'hidden');
			}
